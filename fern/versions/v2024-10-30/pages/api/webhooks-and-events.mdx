...
The first step is to create an Event Subscription via the [API](/v2024-10-30/api/event-subscriptions/create). As part of this, you specify a URL for a server endpoint that will receive real-time events. Chariot will then send HTTPS requests to that URL to notify you of activity for certain events.

When a noteworthy event happens, Chariot first generates an [Event](/v2024-10-30/api/events/get). Next, we'll send a POST request to your endpoint.

<Aside>
  The body of the `POST` webhook request will be the same as the API representation of the [Event](/v2024-10-30/api/events/get) resource.
  ...
  - `organization.created`: This event is triggered when an organization is created.
  - `organization.updated`: This event is triggered when an organization is updated.
  - `address.created`: This event is triggered when an address is created.
  - `address.updated`: This event is triggered when an address is updated.
  ...
  - `grant_intent.created`: This event is triggered when a grant intent is created.
  - `grant_intent.updated`: This event is triggered when a grant intent is updated.
</Aside>
...
If you don't want to use webhooks, or need to do some batch processing, you can also request Events from the Chariot API using the [List Events API](/v2024-10-30/api/events/list). Events will remain in the Chariot system for up to 30 days.
...
<Note>
In order to verify signatures for webhooks, you need to specify the signing secret that you will use in the [Create Event Subscription API](/v2024-10-30/api/event-subscriptions/create).
</Note>
...
You can [Update an Event Subscription](/v2024-10-30/api/event-subscriptions/update) to change the event category for a webhook endpoint with the API.
...
See the section above on [verifying webhook signatures](/v2024-10-30/api/webhooks#secure-your-webhooks)
