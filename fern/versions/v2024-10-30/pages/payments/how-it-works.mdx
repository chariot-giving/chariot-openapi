...
You can use the response to verify the details of the `Organization` to ensure it is the correct one.
...
Chariot will fund the payment by debiting your operating bank account and crediting the Chariot FBO account.
After Chariot processes and submits the payment, you should receive a `payment.updated` webhook event.
...
If a Nonprofit does not have an account with Chariot, they can still receive payments via Chariot's Payments API.
