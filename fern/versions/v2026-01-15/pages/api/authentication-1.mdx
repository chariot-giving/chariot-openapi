...
Authentication to the API is performed via HTTP Bearer Token. Send your API key in the `Authorization` header:

```http
Authorization: Bearer <API_KEY>
```

API keys are server-side credentials. Never put one in browser code, a mobile app, a public repository, or a log. Sandbox and production use separate keys, and each only works against its own host.
...
<Aside>
  <CodeBlocks>
    ...
    ```
    ```java Java
    HttpResponse<String> response = Unirest.get("https://api.givechariot.com/v1/nonprofit/530196605")
    ...
    curl_setopt_array($curl, array(
      CURLOPT_URL => "https://api.givechariot.com/v1/nonprofit/530196605",
