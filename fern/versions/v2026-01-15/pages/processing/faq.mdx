...
<AccordionGroup>
    <Accordion title="How do I map dashboard fields to the API?">
        The table below maps common fields from the Chariot Dashboard donation export to their corresponding API fields on the [Donation](/api/donations/list) object.

        | Dashboard Field | API Field | Notes |
        |---|---|---|
        | Additional Details | `platform.metadata` | Key-value metadata from the platform that facilitated the donation. |
        | Company | `corporate_match.company_name` | The name of the company that matched the donation. |
        | External ID | `donor_advised_fund_grant.sponsor_grant_id` | The grant identifier from the DAF sponsor. |
        | CRM Status / Internal Note | `properties` | Custom properties assigned to the donation via the Properties API. |
        | DAFpay Metadata | `initiation.dafpay_metadata` | Key-value pairs passed to DAFpay during the donation initiation. |
        | DAFpay Form | `initiation.dafpay_form` | The display name of the DAFpay form. Populated for both integrated and unintegrated sources. |
        | DAFpay Type | `initiation.integration_type` | `INTEGRATED` or `UNINTEGRATED`. |
        | Canceled | `initiation.canceled_at` | If non-null, the donation is tied to a canceled grant initiation. |
        | Donor Amount | `corporate_match.individual_contribution_amount` | The individual donor's contribution amount (for corporate match donations). |
        | Match Amount | `corporate_match.match_amount` | The company's matching contribution amount. |
        | Deposit Type | Requires a separate call to the [Deposit API](/api/deposits/get) using `settlement.deposit_id` from the donation. | The deposit contains transfer method and payment source details. |
    </Accordion>
...
<Info>
  Chariot is a financial technology company, not a bank. Chariot Accounts come
  with a Demand Deposit Account through our banking services partner, Column
  N.A., Member FDIC. Deposits in Chariot Accounts are eligible for FDIC
  insurance up to $250,000 per depositor, for each insurable capacity in which
  the account is held.
</Info>
