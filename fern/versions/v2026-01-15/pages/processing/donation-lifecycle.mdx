...
<AccordionGroup>
    <Accordion title="What if a deposit is returned after I sent an acknowledgment?">
        ...
        **Prevention:** Always wait for `deposit.updated` and ensure the Deposit status is `complete` before sending acknowledgments.
    </Accordion>
    ...
    <Accordion title="How do I handle partial deposits?">
        ...
        Each payout creates a separate deposit with its own set of donations. Use the `payment_source_id` to reconcile across deposits.
