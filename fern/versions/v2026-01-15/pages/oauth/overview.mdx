...
When Chariot registers your application, you'll receive a `client_id` and `client_secret`. Your server authenticates with Chariot using HTTP Basic Auth with these credentials.

Your client secret should be kept secure and should never be exposed to the public.

## Scopes

Scopes define what your application can access. Specify them as a space-separated string when requesting authorization. Available scopes will be configured during client registration.
...
| `redirect_uri`          | Yes         | URL to redirect back to (must match a registered URI)                                             |
