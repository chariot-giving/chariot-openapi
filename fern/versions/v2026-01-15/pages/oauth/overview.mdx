...
<Warning>
  ...
  name, a logo, and the callback URLs you want registered — the name and logo
  are shown to nonprofits on the consent screen, and only the registered
  callback URLs can be used as a `redirect_uri`.
</Warning>
...
Scopes define what your application can access. Specify them as a space-separated string when requesting authorization. The scopes your client is allowed to request are fixed during client registration — requesting a scope outside that set will fail the authorization request.

**Grantmaking**

| Scope                        | Grants                                                                             |
| ---------------------------- | ---------------------------------------------------------------------------------- |
| `read:payouts`               | Read and list disbursements and inbound transfers                                  |
| `create:payouts`             | Create disbursements, approve, cancel, and stop them, and create inbound transfers |
| `read:nonprofits`            | Search and read recipient organizations                                            |
| `read:events`                | Read events and webhook subscriptions                                              |
| `create:event_subscriptions` | Create and update webhook subscriptions                                            |
...
| `offline_access` | Accepted for compatibility with OIDC clients. Refresh tokens are issued regardless           |

<Note>
  A token only receives the permissions your client, the authorization, and the
  consenting user's role all have in common. Because only Owners and Admins hold
  every permission above, ask the nonprofit to have an Owner or Admin approve
  your application — a user on a more limited role can complete the flow and
  still receive a token with less access than you requested.
</Note>
...
https://dashboard.givechariot.com/oauth/authorize
  ...
  &response_type=code
  &scope=openid%20read:payouts%20create:payouts
