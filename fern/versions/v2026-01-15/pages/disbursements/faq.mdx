...
<AccordionGroup>
    <Accordion title="What if a nonprofit isn't listed on Chariot?">
        ...
        If a nonprofit organization is not found through Chariot's [Search Organizations](/api/organizations/search) endpoint,
        you can submit a [Verification Request](/api/verification-requests/create) to have Chariot's compliance team review and verify the organization.

        The request requires the organization's EIN, name, and mailing address. You can optionally provide a website URL and contact email.

        You can track the status of the verification request by polling the [GET Verification Request](/api/verification-requests/get) endpoint
        or by subscribing to `verification_request.updated` [webhook events](/api/webhooks).

        For more details, see [Disbursing to Unlisted Organizations](/guides/disbursements/how-it-works#disbursing-to-unlisted-organizations).
    </Accordion>
    ...
    <Accordion title="What does the onboarding process look like for nonprofits?">
        ...
        <Steps>
            ...
            The nonprofit provides verifying information to Chariot to activate a <Tooltip tip="Chariot is a financial technology company, not a bank. Chariot Accounts come with a Demand Deposit Account through our banking services partner, Column N.A., Member FDIC. Deposits in Chariot Accounts are eligible for FDIC insurance up to $250,000 per depositor, for each insurable capacity in which the account is held.">Chariot Deposit Account</Tooltip>.
