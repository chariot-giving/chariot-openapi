...
| `awaiting_verification` | The disbursement is awaiting verification from Chariot. This status is used when disbursing to an organization created via a [Verification Request](/api/verification-requests/create) without setting `bypass_chariot_organization_verification` to `true`. The disbursement will automatically proceed once the organization is verified. |
