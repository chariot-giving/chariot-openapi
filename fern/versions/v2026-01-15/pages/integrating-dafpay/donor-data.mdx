...
That fill is called Express Checkout. It needs no setup, but it only works when the button sits above your contact fields, and it's best-effort: some providers don't expose a donor's address or phone at all, so those fields come back empty and the donor is asked for them in the window. You can try it on the [demo page](https://secure.dafpay.com/demo) with the Express toggle on.

You still have to pass the amount yourself, because Express Checkout doesn't supply it.
...
If a value runs past its maximum we truncate it instead of failing the request.

Send `anonymous` when a donor chooses to give anonymously, so the DAF provider anonymizes the grant properly.

<Warning title="Send the donor's details, not your customer's">
Everything in `onDonationRequest` describes the person giving. On a multi-tenant platform it's easy to pass the nonprofit's admin contact or the account holder instead, which then arrives on the gift as the donor.
</Warning>
...
Platforms usually break this in one of two ways. Either a custom button calls your own server before triggering the popup, or `onDonationRequest` goes off to fetch giving history or check eligibility. Move that work earlier, before the donor clicks.
