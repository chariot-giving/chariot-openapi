...
<AccordionGroup>
    <Accordion title="What information is needed to register a nonprofit for DAFpay?">
    To register a nonprofit, you will first [Search for the Organization](/api/organizations/search) using the nonprofit's EIN or legal name.
    Once, you have found the Organization, you should [Create a Connect](/api/connects/create) and provide contact information for
    the organization.
    ...
    If the Create Nonprofit API returns an error, it means the nonprofit is not yes eligible to receive donations from DAFpay.

    This could be because the nonprofit was not found in Chariot's verified database or because they are not eligible for Donor Advised Fund gifts. The error code in the response will indicate the specific reason the route failed for the given nonprofit.

    Please reach out to contact@givechariot.com if you have any questions or concerns about this evaluation.
...
<AccordionGroup>
    ...
    <Accordion title="What are the test credentials?">
        You can use the following credentials for testing in Sandbox:
...
<AccordionGroup>
    <Accordion title="How do nonprofits receive funds?">
    ...
    <Accordion title="How do multi-organization grants work?">

    Multi-organization grants allow donors to give to multiple nonprofits in a single DAFpay session. Pass a comma-separated list of CIDs to `<chariot-connect />`, provide grant details in `onDonationRequest`, then call the [Create Grants](/api/grants/create-grants) endpoint with the `workflowSessionId`.

    **Important:** Chariot cannot guarantee "all or nothing" transactions—some grants may succeed while others fail.

    See [Multi-Organization Grants](/guides/dafpay/integrating-dafpay/integration#multi-organization-grants) for details.
