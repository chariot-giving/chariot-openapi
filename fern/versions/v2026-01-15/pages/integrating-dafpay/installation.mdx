...
The button is a small JavaScript package. Load it from the Chariot CDN, or install from [NPM](https://www.npmjs.com/package/react-chariot-connect) or Yarn. CDN builds have to load from `https://cdn.givechariot.com` at runtime — bundling it or hosting a copy breaks it.
...
List DAFpay alongside your primary payment methods — credit card, bank transfer, Apple Pay — and put it above the amount and contact fields, or [Express Checkout](/guides/dafpay/integrating-dafpay/donor-data) can't prefill.
...
For a custom theme, register an object with the properties you want to change, using [Tailwind CSS](https://tailwindcss.com/) values. Only the three below are adjustable — allowing more would mean shipping all of Tailwind in the bundle, at a load-time cost to every donor.
