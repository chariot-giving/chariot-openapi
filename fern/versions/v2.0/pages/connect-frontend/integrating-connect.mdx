...
| frequency  | The recurring frequency of the DAF grant (if it's a recurring gift). This parameter allows the following possible enum values: `ONE_TIME`, `MONTHLY`. If not provided, the grant will default to a `ONE_TIME` grant. For more information on recurring donations, please see the [section below.]\\(## Recurring Donations) | string (optional) |
...
After you submit your own donation form, don't forget to call the [Create Recurring Grant](ref:create-recurring-grant) API to complete the transaction. Note that if your platform enables recurring donations through Chariot, you will need to differentiate which of Chariot's APIs you call to capture the donation based on the information of the `CHARIOT_SUCCESS` event.
