...
The first step is to create an Event Subscription via the [API](/v2023-01-01/api/event-subscriptions/create). As part of this, you specify a URL for a server endpoint that will receive real-time events. Chariot will then send HTTPS requests to that URL to notify you of activity for certain events.

When a noteworthy event happens, Chariot first generates an [Event](/v2023-01-01/api/events/get). Next, we'll send a POST request to your endpoint.

<Aside>
  The body of the `POST` webhook request will be the same as the API representation of the [Event](/v2023-01-01/api/events/get) resource.
...
If you don't want to use webhooks, or need to do some batch processing, you can also request Events from the Chariot API using the [List Events API](/v2023-01-01/api/events/list). Events are retained and are not deleted. The delivery history shown in the dashboard covers the last 30 days.
...
<Note>
In order to verify signatures for webhooks, you need to specify the signing secret that you will use in the [Create Event Subscription API](/v2023-01-01/api/event-subscriptions/create).
</Note>
...
See the section above on [verifying webhook signatures](/v2023-01-01/api/webhooks#secure-your-webhooks)
