...
If you don't want to use webhooks, or need to do some batch processing, you can also request Events from the Chariot API using the [List Events API](/v2023-01-01/api/events/list). Events will remain accessible from Chariot for up to 30 days.
...
Use the `categories` field to choose which events a subscription receives. You can [Update an Event Subscription](/v2023-01-01/api/event-subscriptions/update) to change its categories or its status. A subscription's URL is set when you create it and can't be changed.
