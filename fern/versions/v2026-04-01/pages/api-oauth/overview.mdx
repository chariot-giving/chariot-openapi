...
| **Sandbox** | `https://dashboard.givechariot.com/oauth/sandbox/authorize` | `https://sandboxapi.givechariot.com/auth/oauth/token` |

Sandbox authorization is served from the production dashboard — only the path differs, so donors sign in at the hostname they already know. The token, UserInfo, and discovery endpoints are on the sandbox API host.

Each environment issues its own `client_id` and `client_secret`. Sandbox credentials cannot be used in production and vice versa.

Rather than hardcoding these, read them from the discovery document for your environment:

| Environment | OIDC configuration |
| ----------- | ------------------ |
| **Production** | `https://api.givechariot.com/.well-known/openid-configuration` |
| **Sandbox** | `https://sandboxapi.givechariot.com/.well-known/openid-configuration` |

## Scopes

Scopes define what your application can access. Specify them as a space-separated string when requesting authorization. The scopes your client is allowed to request are fixed during client registration.

| Scope            | Grants                                                                                       |
| ---------------- | -------------------------------------------------------------------------------------------- |
| `read_only`      | Read access to the nonprofit's data across the Chariot API                                   |
| `read_write`     | Read and write access across the Chariot API                                                 |
...
<Note>
  ...
  consenting user's role all have in common.
</Note>
