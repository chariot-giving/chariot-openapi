...
**Donor Accounts** — A Donor Account represents a DAFpay donor identity linked to your organization. It holds an optional `external_id` — your internal identifier for the user's account. A Donor Account must be approved before its Grant Requests can be processed. A Donor Account can be associated with multiple Funds; a Fund represents a specific giving pool within a donor's DAF.
