...
**Donor Accounts** — A Donor Account represents a DAFpay donor identity linked to your organization. It holds the donor's verified `email`, `status`, the `fund_id` of the giving pool they're using, and an optional `external_id` to link them to your internal records. An Account must reach `approved` status before its Grant Requests can be processed.

**Grant Requests** — Submitted when a donor uses DAFpay on a nonprofit's donation form. Grant Requests are tied to a specific Account and can only be approved or rejected once the Account is `approved`.
...
- **[DAF-Initiated Setup](/guides/dafpay/donor-accounts/account-lifecycle#daf-initiated-setup)** — The DAF creates an Account and token in advance and gives the code to the donor via their portal.
