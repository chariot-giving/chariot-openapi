...
A donor can set up a `MONTHLY` gift from their DAF instead of a one-time one.

The DAF provider owns the schedule. Chariot records the initial grant with `frequency: MONTHLY` and doesn't create a new grant each month, and we aren't notified if the donor cancels or changes it — that happens in their provider's portal.

A recurring session returns a `recurringGrantIntent` on `CHARIOT_SUCCESS` rather than a `grantIntent`.
