...
You can see the destinations Chariot holds for a nonprofit with [List Payment Methods](/api/payment-methods/list): its Chariot Deposit Account, a verified bank account, or a verified mailing address, each with a status. If Chariot has repeatedly been unable to reach a nonprofit and holds no active payment method for it, Chariot will not accept new disbursements to that nonprofit until a payment method is verified. If you have a mailing address for the nonprofit, submit it with [Create a Payment Method](/api/payment-methods/create); it starts as `pending_verification` and becomes `active` once a Chariot operator verifies it. Subscribe to the `payment_method.updated` [webhook event](/api/webhooks) to be notified when that happens.
