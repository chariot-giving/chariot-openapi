...
Each organization also carries `payment_preferences`. When a nonprofit has retired an EIN in favor of another, `accepts_payments` is `false` and `redirect_to` names the organization to pay instead; create the disbursement to that organization.
...
You can track the progress of the verification by polling the [GET Verification Request](/api/verification-requests/get) endpoint or by subscribing to `verification_request.updated` [webhook events](/api/webhooks). The status will transition through:
...
You can see the destinations Chariot holds for a nonprofit with [List Payment Methods](/api/payment-methods/list): its Chariot Deposit Account, an account at a US bank, or a postal address that receives checks, each with a status. If Chariot has repeatedly been unable to reach a nonprofit and holds no active payment method for it, Chariot will not accept new disbursements to that nonprofit until a payment method is verified. If you have a postal address for the nonprofit, submit it with [Create a Payment Method](/api/payment-methods/create); it starts as `pending_verification` and transitions to `active` once Chariot verifies it. See the [Payment Methods](/guides/disbursements/payment-method-lifecycle) guide, and subscribe to the `payment_method.updated` [webhook event](/api/webhooks) to be notified when that happens.
