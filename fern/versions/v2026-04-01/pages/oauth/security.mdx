...
The integration is designed so that Chariot holds **scoped, revocable tokens — never donor credentials**. This page collects the security practices we recommend (and a few we require) on top of the [OAuth & Token Requirements](/guides/oauth/oauth-requirements). If you run a standards-compliant identity provider, most of these are configuration rather than code.
...
- Tokens issued to Chariot should authorize only the capabilities in [API Endpoints](/guides/oauth/api-endpoints): the donor's profile, their funds and grantable balances, grant creation, and grant status.
...
- Issue **short-lived access tokens** (15 minutes recommended) and long-lived refresh tokens per [Connection Lifetime](/guides/oauth/oauth-requirements#connection-lifetime).
