...
* [ ]  Authorization Code flow with `state` round-trip (PKCE recommended — see [Security Best Practices](/guides/oauth/security#pkce))
* [ ]  Scopes: `openid profile email offline_access` (plus any custom scopes you require)
* [ ]  Tokens scoped to DAF granting only — never to investment, banking, or brokerage systems (see [Scope of access](/guides/oauth/security#scope-of-access))
* [ ]  ID Token with stable `sub` and `email` claims
* [ ]  Refresh tokens rotated with each use (preferred) or expiring after 13+ months — see [Connection Lifetime](/guides/oauth/oauth-requirements#connection-lifetime)
