...
* [ ]  Authorization Code flow with `state` round-trip (PKCE recommended — see [Security Best Practices](/guides/oauth-integration/security#pkce))
* [ ]  Scopes: `openid profile email offline_access` (plus any custom scopes you require)
* [ ]  Tokens scoped to DAF granting only — never to investment, banking, or brokerage systems (see [Scope of access](/guides/oauth-integration/security#scope-of-access))
* [ ]  ID Token with stable `sub` and `email` claims
* [ ]  Refresh tokens rotated with each use (preferred) or expiring after 13+ months — see [Connection Lifetime](/guides/oauth-integration/oauth-requirements#connection-lifetime)
...
No. In the OAuth model, grants are created directly in your system via your Create Grant API and flow through your normal internal review pipeline. The [Donor Accounts](/guides/donor-accounts/overview) decisioning API is for DAFs without their own APIs.
