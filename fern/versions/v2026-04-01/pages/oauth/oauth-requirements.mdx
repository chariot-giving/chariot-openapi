...
Chariot always includes PKCE (S256) parameters. Supporting PKCE is recommended as defense-in-depth — see [Security Best Practices](/guides/oauth-integration/security#pkce) — but not required: Chariot is a confidential client, and servers that don't support PKCE can ignore the parameters.
...
- API base URLs and documentation for the [resource endpoints](/guides/oauth-integration/api-endpoints)
...
| Refresh token rotation on each use (preferred) — or **13+ month expiry** | Strongly recommended |
