...
<Aside>
  ...
  Event names follow the pattern `{resource}.{event_type}` where `event_type` is either `created` or `updated`. Status transitions (for example, a Donor Account moving to `approved` or `rejected`) are delivered as `{resource}.updated` events — fetch the object to read its current status.
  ...
  - `donor_account.created`: This event is triggered when a [Donor Account](/api/donor-accounts) is created.
  - `donor_account.updated`: This event is triggered when a Donor Account is updated. This can happen when the status transitions to `approved`, `rejected`, or `disabled`.
  - `authorization_token.created`: This event is triggered when an [Authorization Token](/api/authorization-tokens) is created.
  - `authorization_token.updated`: This event is triggered when an Authorization Token is updated. This can happen when the status transitions to `verified`, `revoked`, or `expired`.
  - `grant_request.created`: This event is triggered when a [Grant Request](/api/grant-requests) is created.
  - `grant_request.updated`: This event is triggered when a Grant Request is updated. This can happen when the status transitions to `pending`, `submitted`, or `rejected`.
  ...
  - `donation.created`: This event is triggered when a donation is created.
  - `donation.updated`: This event is triggered when a donation is updated.
  - `deposit.created`: This event is triggered when a deposit is created.
  - `deposit.updated`: This event is triggered when a deposit is updated. This can happen when the status of the deposit or underlying transfer changes.
  - `verification_request.created`: This event is triggered when a verification request is created.
  - `verification_request.updated`: This event is triggered when a verification request is updated.
</Aside>
...
If you don't want to use webhooks, or need to do some batch processing, you can also request Events from the Chariot API using the [List Events API](/api/events/list). Events are retained and are not deleted. The delivery history shown in the dashboard covers the last 30 days.
...
Compute an HMAC with the SHA256 hash function. Use the Event Subscription's `signing_secret` as the key, and use the `signed_payload` string as the message.
...
An Event Subscription's `categories` field holds the full list of event categories it receives. The original `category` field is still present and populated with the first entry in `categories`, so it keeps working for existing integrations. You can [Update an Event Subscription](/api/event-subscriptions/update) to change its list of categories or its status with the API. The URL can't be changed after creation. Creating a subscription for a URL that already has an active subscription returns a 409 error naming the existing one. Update that subscription's categories instead of creating a duplicate.
