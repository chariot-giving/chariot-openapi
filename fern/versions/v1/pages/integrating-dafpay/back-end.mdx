...
If you go with manual grant intents (option 1), you will need to render the trackingID and DAF provider url in the thank you redirect page so the donor will be able to add the trackingID on their gift. This information is included on the user exit event `UNINTEGRATED_GRANT_CONFIRMED` for.