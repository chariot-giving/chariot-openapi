...
<AccordionGroup>
    ...
    To register a nonprofit using the [Create nonprofit](/api/nonprofits/create) endpoint you need:
     ...
     2. Contact email (only be used to reach out to the nonprofit to claim their account)
...
<AccordionGroup>
    ...
    <Accordion title="Can we add an optional tip?">
        ...
        DAF giving can not be used to give to a for-profit entity which means tipping to the fundriaisng platform is not allowed.

        If a donor has selected to enable DAFPay, you must suppress the option to add a tip. 

        See [Go Fund Me](https://www.gofundme.com/f/help-leo-aid-hurricane-milton-and-helene-victims/donate?source=btn_donate) for an example of how to remove the tip funcitonality when DAFpay is selected. 
    </Accordion>
...
<AccordionGroup>
    <Accordion title="What best practices for placing the DAFpay button ona  donation form?">
