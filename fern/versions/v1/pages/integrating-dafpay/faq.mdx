...
<AccordionGroup>
    <Accordion title="What is the sandbox API url">
        For testing you can use the sandbox API url: `https://chariot-sandbox.us.auth0.com/oauth/token`
    </Accordion>
    <Accordion title="What EIN should I use for testing?">
        You can use any valid EIN for testing in sandbox.
    </Accordion>
...
<AccordionGroup>
    ...
    <Accordion title="Can we add an optional tip?">
        ...
        See [Go Fund Me](https://www.gofundme.com/f/help-leo-aid-hurricane-milton-and-helene-victims/donate?source=btn_donate) for an example of how to remove the tip functionality when DAFpay is selected. 
    </Accordion>
...
<AccordionGroup>
    ...
    <Accordion title="What are manual grant intents?">

    Manual grant intents is an optional feature where DAFpay can facilitate donations from unintegrated DAF providers.

    If configured, if a user selects a DAF provider that is not integreated with DAFpay, the modal can collect the donation amopunt and donor information so the non porfit can be notified in real time. The donor is then given the trackingID and asked to included in the the note when they make their donaiton through their DAf portal that way  it can be used for reconciliation.

    Since the donor has to leave the nonprofit site to actualy submit the donation we do not have visibility into wheather or not the donation was actually made.
