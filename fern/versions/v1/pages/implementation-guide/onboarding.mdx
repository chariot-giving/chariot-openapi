...
The following steps outline the process of onboarding a nonprofit organization to Chariot:

## Register Nonprofit via API

- [Create Nonprofit](/api-reference/nonprofits/create): Call this route to create a nonprofit organization in our system using the organization’s EIN and email address
...
- [Create Connect](/api-reference/connects/create): After creating the nonprofit, call this route using the `nonprofitId` to generate a Connect record, which will allow you to enable the DAFpay button on the nonprofit’s donation form.
...
## Claim Chariot Account
    - Upon calling the [Create Connect](/api-reference/connects/create) route, Chariot will automatically email the nonprofit (using the email provided during nonprofit creation) with instructions on how to claim their Chariot account.
