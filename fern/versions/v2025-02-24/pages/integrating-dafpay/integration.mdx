...
<Error title="Preventing Popup Blocking in Mobile Browsers">
...
Based on experimental evidence, mobile browsers allow a maximum of 1000ms between the user action and popup launch before blocking occurs. To ensure a seamless integration with DAFpay, platforms must be mindful of potential pitfalls when handling donation requests.

Common Scenarios Where Popup Blocking Occurs:
1. Custom Button with Shadow DOM and Delayed API Calls
	•	If your platform has implemented a custom donation button using the Shadow DOM and manually triggers the DAFpay popup, be cautious about adding intermediate API or network requests before invoking the popup.
	•	Example issue:
	•	User clicks the platform’s custom button.
	•	The platform makes an API call to its own servers before triggering the DAFpay popup.
	•	If the delay exceeds 1000ms, the browser may block the popup.
	•	Solution:
	•	Ensure that all network requests complete before the Shadow DOM event is triggered.
	•	Minimize intermediate operations to prevent unnecessary delays.
2. Async Operations in onDonationRequest Callback
	•	Some platforms rely on the built-in Chariot Connect (DAFpay) button to open the popup. However, they introduce additional asynchronous operations in the onDonationRequest callback, such as:
	•	Fetching user donation history
	•	Verifying donor eligibility via API calls
	•	Retrieving pre-filled form data
	•	Issue: Since onDonationRequest allows for async operations, any delay beyond 1000ms increases the risk of the popup being blocked.
	•	Solution:
	•	Minimize async logic inside onDonationRequest.
	•	If additional data fetching is required, perform it before triggering the donation request or use background loading strategies.
	•	We are evaluating an alternative API that avoids async operations in this flow to mitigate this issue in future releases.
...
After you submit your own donation form, do not forget to call the [Create Grant](/api/grants/create) API to complete the transaction.
</Warning>
...
3. Have your backend call Chariot's [Create Grant](/api/grants/create) API to complete the grant (and capture the grant intent).
