...
[1] Custom Button with Shadow DOM and Delayed API Calls
  ...
  For example, if a user clicks the platform’s custom button and the platform makes an API call to its own servers before triggering the DAFpay popup, if the delay exceeds 1000ms, the browser may block the popup.
