...
<AccordionGroup>
    <Accordion title="Account Transfer">
        Payments sent from one Chariot Deposit Account to another Chariot Deposit Account<span title="Chariot is a financial technology company, not a bank. Chariot Accounts come with a Demand Deposit Account through our banking services partner, Column N.A., Member FDIC. Deposits in Chariot Accounts are eligible for FDIC insurance up to $250,000 per depositor, for each insurable capacity in which the account is held."><sup><Icon icon="fa-regular fa-circle-info" size="2.5"/></sup></span> can be made 24 hours a day, 7 days a week and, once approved, will settle instantly.
    </Accordion>
    <Accordion title="ACH">
        ACH submission represents a credit (push) payment using the Automated Clearing House (ACH) network.
        This transfer method is used if the receiving organization is not available for Account Transfers.
