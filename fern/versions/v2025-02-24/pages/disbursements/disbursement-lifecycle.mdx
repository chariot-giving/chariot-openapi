...
Just-In-Time disbursements enable grantmakers to create and send disbursements without needing to pre-fund their Chariot Deposit Account<span title="Chariot is a financial technology company, not a bank. Chariot Accounts come with a Demand Deposit Account through our banking services partner, Column N.A., Member FDIC. Deposits in Chariot Accounts are eligible for FDIC insurance up to $250,000 per depositor, for each insurable capacity in which the account is held."><sup><Icon icon="fa-regular fa-circle-info" size="2.5"/></sup></span>. When a disbursement with auto-funding enabled is approved, Chariot automatically creates an inbound transfer for the exact disbursement amount by immediately initiating an ACH debit.
