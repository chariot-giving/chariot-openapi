...
If a nonprofit has a DDA Account** with Chariot, they can receive payments electronically to their verified bank account.
...
If a nonprofit does not have a DDA Account with Chariot, they can still receive payments via Chariot's Disbursements.
In this case, Chariot will mail a paper check to the nonprofit's verified postal address.
We strongly recommend that nonprofits sign up for a DDA account** with Chariot to receive future disbursements electronically.
...
This option is ideal for grantmakers that do not want to manage their own Accounts Payable systems and/or want to offload the responsibility of
...
\* _Chariot is a financial technology company, not a bank. Chariot Accounts come with a Demand Deposit Account through our banking services partner, Column N.A., Member FDIC. Deposits in Chariot Accounts are eligible for FDIC insurance up to $250,000 per depositor, for each insurable capacity in which the account is held._

** _Fees may apply to other actions taken in your Chariot account. See the full schedule here._