...
<Note>
**Inbound transfer settlement times:** ACH debits take **3-4 business days** to settle and ACH credits take **1-2 business days**. Funds are not available for disbursements until the transfer completes.
</Note>

<Tip>
**Pre-funding recommendation:** We recommend maintaining a rolling balance of **1-4 weeks** of expected disbursement volume in your Chariot Deposit Account. This allows disbursements to execute immediately without waiting for inbound transfers to settle. Even a 1-week buffer can significantly reduce grant timelines.
</Tip>

If pre-funding isn't feasible, you can use [Just-In-Time (JIT) funding](/guides/disbursements/disbursement-lifecycle#just-in-time-jit-disbursements) to automatically create an inbound transfer when a disbursement is approved. Note that JIT disbursements still incur the ACH settlement delay before funds are available.
